...
import multipazphotoidgettingstartedsample.composeapp.generated.resources.Res
import multipazphotoidgettingstartedsample.composeapp.generated.resources.compose_multiplatform
...
class App {
    ...
    val appName = "Multipaz PhotoID Getting Started Sample"
    ...
    suspend fun init() {
        if (!isInitialized) {
            ...
            if (documentStore.listDocuments().isEmpty()) {
                ...
                // Debug: Inspect the namespace
                println("=== NAMESPACE INSPECTION DEBUG ===")
                val credentials2 = document.getCredentials()
                credentials2.forEach { cred ->
                    if (cred is MdocCredential) {
                        println("Credential docType: ${cred.docType}")
                        
                        try {
                            // Decode the issuer provided data
                            val issuerSigned = Cbor.decode(cred.issuerProvidedData)
                            println("Issuer signed data keys: ${issuerSigned.asMap.keys}")
                            
                            // Get the namespaces
                            val nameSpaces = issuerSigned["nameSpaces"]
                            println("NameSpaces type: ${nameSpaces::class.simpleName}")
                            
                            // Parse the namespaces
                            val issuerNamespaces = IssuerNamespaces.fromDataItem(nameSpaces)
                            println("Number of namespaces: ${issuerNamespaces.data.size}")
                            
                            // Inspect each namespace
                            issuerNamespaces.data.forEach { (namespaceName, namespaceData) ->
                                println("--- Namespace: $namespaceName ---")
                                println("Data elements count: ${namespaceData.size}")
                                namespaceData.forEach { (elementName, element) ->
                                    println("  Element: $elementName")
                                    println("    Data element identifier: ${element.dataElementIdentifier}")
                                    try {
                                        val dataItem = element.toDataItem()
                                        println("    Actual value: $dataItem")
                                        println("    Value type: ${dataItem::class.simpleName}")
                                    } catch (e: Exception) {
                                        println("    Could not get actual value: ${e.message}")
                                    }
                                }
                            }
                            
                        } catch (e: Exception) {
                            println("Error parsing namespaces: ${e.message}")
                            e.printStackTrace()
                        }
                    }
                }
                println("=== END NAMESPACE INSPECTION DEBUG ===")


                
                // Debug: Check what document type was actually created
                println("=== DOCUMENT TYPE DEBUG ===")
                println("PhotoID.PHOTO_ID_DOCTYPE = ${PhotoID.PHOTO_ID_DOCTYPE}")
                val credentials = document.getCredentials()
                println("Document has ${credentials.size} credentials")
                credentials.forEach { cred ->
                    if (cred is MdocCredential) {
                        println("MdocCredential docType = ${cred.docType}")
                        println("Matches PhotoID.PHOTO_ID_DOCTYPE? ${cred.docType == PhotoID.PHOTO_ID_DOCTYPE}")
                    } else {
                        println("Credential is not MdocCredential: ${cred::class.simpleName}")
                    }
                }
                println("=== END DEBUG ===")
                ...
                if (!blePermissionState.isGranted) {
                    ...
                    when (state.value) {
                        PresentmentModel.State.IDLE -> {
                            ShowQrButton(deviceEngagement, coroutineScope)
                        }
                        ...
                        PresentmentModel.State.COMPLETED -> {
                            Presentment(
                                appName = "Multipaz PhotoID Getting Started Sample",
    ...
    /**
     * Debug function to inspect document namespaces and data elements
     */
    private suspend fun debugDocumentNamespaces(doc: Document) {
        println("=== DOCUMENT DEBUG INSPECTION ===")
        println("Document: ${doc.metadata.displayName}")
        println("Document ID: ${doc.identifier}")
        val creds = doc.getCredentials()
        creds.forEach { cred ->
            if (cred is MdocCredential) {
                println("  Credential docType: ${cred.docType}")
        ...
        println("=== END DOCUMENT DEBUG INSPECTION ===")
    }

    @Composable
    private fun ShowQrButton(showQrCode: MutableState<ByteString?>, coroutineScope: CoroutineScope) {
        Column(
            modifier = Modifier.fillMaxSize(),
            verticalArrangement = Arrangement.Center,
            horizontalAlignment = Alignment.CenterHorizontally
            ...
            Button(
                onClick = {
                    coroutineScope.launch {
                        println("=== MANUAL DEBUG INSPECTION ===")
                        val docs = documentStore.listDocuments()
                        println("Found ${docs.size} existing documents")
                        docs.forEach { docName ->
                            val doc = documentStore.lookupDocument(docName)
                            if (doc != null) {
                                debugDocumentNamespaces(doc)
                            }
                        }
                        println("=== END MANUAL DEBUG INSPECTION ===")
                    }
                }
            ) {
                Text("Debug Documents")
            }
        }
    }
